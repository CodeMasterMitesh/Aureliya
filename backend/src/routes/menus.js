...
r.get('/menus', async (req, res) => {
  try {
    const mains = await MainMenu.aggregate([
      { $sort: { order: 1, name: 1 } },
      {
        $lookup: {
          ...
          let: { mid: '$_id' },
          pipeline: [
            { $match: { $expr: { $and: [ { $eq: ['$main_menu_id', '$$mid'] }, { $eq: ['$parent_id', null] } ] } } },
            { $sort: { order: 1, name: 1 } },
          ],
          as: 'submenus'
        }
    ...
    function buildTree(node){
      ...
      return { ...node, submenus: kids.sort((a,b)=> (a.order-b.order) || a.name.localeCompare(b.name)).map(buildTree) }
    }
    ...
    const items = mains.map(m => {
      ...
      return {
        ...
        icon: m.icon,
        order: m.order,
        submenus: tops.sort((a,b)=> (a.order-b.order)||a.name.localeCompare(b.name)).map(buildTree)
      }
    })
    res.json({ items })
  } catch (e){
...
r.get('/main-menus', [
  ...
  const [items, total] = await Promise.all([
    MainMenu.find(q).sort({ order:1, name:1 }).skip((page-1)*limit).limit(limit).lean(),
...
r.get('/sub-menus', [
  ...
  const [items, total] = await Promise.all([
    SubMenu.find(q).sort({ order:1, name:1 }).skip((page-1)*limit).limit(limit).lean(),
...
r.post('/sub-menus', [
  ...
  const doc = await SubMenu.create({ main_menu_id, parent_id, name, slug: slug||slugify(name), path, order, meta })
  res.status(201).json(doc)
})
